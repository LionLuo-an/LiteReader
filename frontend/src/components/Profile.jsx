...
const UserEquippedBadge = ({ userId, themeColors, fallbackRole, isEInk, isDark }) => {
  ...
  useEffect(() => {
    if (!userId) return;
    fetch('/api/achievements/my', { credentials: 'include' }).then(res => res.json()).then(data => {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className={`${themeColors.card} rounded-[20px] overflow-hidden shadow-sm divide-y ${themeColors.divide}`}>
        ...
        >
          <div className="flex items-center gap-3">
            <FileText className={themeColors.icon} size={22} strokeWidth={1.5} />
...
const Profile = () => {
  ...
  return (
    <div className={`${themeColors.bg} transition-colors duration-200`}>
      ...
      {/* 顶部 Header */}
      {!isFnOSMobile && (
        <div className={`px-4 py-3 flex items-center justify-between sticky top-0 z-10 ${themeColors.headerBg}`}>
          <ArrowLeft className={`w-6 h-6 ${themeColors.textMain}`} onClick={() => window.history.back()} />
          <span className={`text-[17px] font-bold ${themeColors.textMain}`}>我的</span>
          <div className="w-6"></div>
        </div>
      )}

      {/* 移除 min-h-screen 和嵌套的 overflow-y-auto，让外层 Layout 统一处理滚动 */}
