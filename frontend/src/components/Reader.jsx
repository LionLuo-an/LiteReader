...
const Reader = () => {
    ...
    useEffect(() => {
        if (!bookId) return;
        fetch(`/api/notes/${bookId}`, { credentials: 'include' })
            .then(res => res.json())
    ...
    // 阅读时长统计
    useReadingTime(bookId, null, theme);
    ...
    // --- Fullscreen Logic ---
    // iOS Safari 无法使用标准 Fullscreen API（或行为不一致），采用 CSS 覆盖模拟全屏
    // 增强的 iOS 检测（包含 iPadOS 桌面模式）
    const isIOS = /iphone|ipad|ipod/.test(navigator.userAgent.toLowerCase()) || 
        (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);

    // 检测是否在 PWA 模式下运行 (standalone)
        ...
        if (!isNativeFs && !isFullscreen) {
            ...
            try {
                if (requestFs) {
                    ...
                    // Do NOT push state here, rely on native behavior or manual control if needed
                    // Pushing state causes double-back requirement
                } else {
            ...
            // No history.back() needed if we didn't push state
        }
    }, [isFullscreen, isPWA, isIOS, showControls]);
    ...
    // iOS 场景下进入全屏时，确保隐藏顶部导航和底部操作栏
    useEffect(() => {
        // PWA 模式下由用户手动控制显隐，不强制隐藏
        const isPWA = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true;
    ...
    const handleBack = () => {
        ...
        try {
            ...
            fetch(`/api/books/${bookId}/progress`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ scroll_top: scrollTop, chapter_index: index, chapter_title: chapterTitle }),
                keepalive: true,
                credentials: 'include'
            }).catch(console.error);
    ...
    const fetchNotes = async () => {
        try {
            const res = await fetch(`/api/notes/${bookId}`, { credentials: 'include' });
            if (res.ok) {
        ...
        try {
            ...
            if ((fmt === 'epub' || fmt === 'mobi' || fmt === 'azw3') && content) {
                const normalizeAssetUrl = (src) => {
                    ...
                    if (!value || value.startsWith('#') || value.startsWith('http') || value.startsWith('data:') || value.startsWith('blob:')) {
                        return value;
                    }
                    if (value.startsWith('/api/books/')) return value;
                    return `/api/books/${bookId}/image?path=${encodeURIComponent(value.replace(/^\.\//, ''))}`;
                };
    ...
    useEffect(() => {
        ...
        // 增加 beforeunload 监听，确保关闭网页时也能保存
    ...
    const handleTouchMove = (e) => {
        ...
        const container = contentRef.current;
        if (container) {
            // Manual 1:1 scroll tracking
            container.scrollLeft = touchStartRef.current.startScrollLeft - dx;
        }
    };
    ...
    const handleTouchEnd = (e) => {
        ...
        // Thresholds
        if (dt > 500) {
            // If too slow (not a flick), maybe we should still snap to nearest page?
            // Since we essentially implemented "pan", we should snap to nearest.
            // But existing logic returns.
            // With manual drag, if we return, the page stays at dragged position.
            // So we MUST NOT return, or we must snap back.
            // Let's treat it as a small move -> Revert.
            if (viewMode === 'h-page' || viewMode === 'instant') {
                const container = contentRef.current;
                if (container && touchStartRef.current) {
                    const pageStep = container.getBoundingClientRect().width;
                    container.scrollTo({ left: touchStartRef.current.startPageH * pageStep, behavior: 'smooth' });
                }
            }
            return;
        }
        if (Math.max(absDx, absDy) < 30) {
            // Too small -> Revert (Snap back)
            if (viewMode === 'h-page' || viewMode === 'instant') {
                const container = contentRef.current;
                if (container && touchStartRef.current) {
                    const pageStep = container.getBoundingClientRect().width;
    ...
    useEffect(() => {
        ...
        const initReader = async () => {
            ...
            const isMobile = /iphone|ipad|ipod|android|mobile/.test(ua);
            if (isMobile) {
                // iOS: Skip auto-fullscreen (browser limitations make it ineffective)
                // Android: Try Native Fullscreen API
                if (!isIOS) {
                    setTimeout(() => {
                        if (!document.fullscreenElement) {
                            document.documentElement.requestFullscreen().then(() => {
                                // Do NOT push state here, just enter fullscreen
                                // This avoids the double-back requirement
                            }).catch(() => { });
                        }
                    }, 500);
                }
            ...
            try {
                ...
                try {
                    ...
                    if (!hasLocalCache) {
                        ...
                        try {
                            const settingsRes = await fetch(`/api/preferences/reader/settings/${currentDeviceType}`, {
                                credentials: 'include'
                            });
                            if (settingsRes.ok) {
                ...
                if (!tocData) {
                    ...
                    const res = await fetch(`/api/books/${bookId}/toc?stream=true`, {
                        credentials: 'include'
                    });
    ...
    const updateSetting = async (key, value) => {
        ...
        settingsSyncRef.current = setTimeout(() => {
            ...
            fetch(`/api/preferences/reader/settings/${deviceType}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const handleAddToBookshelf = async () => {
        ...
        // 后台静默请求
        try {
            const res = await fetch(`/api/books/bookshelf/${bookId}`, { method: 'POST', credentials: 'include' });
    ...
    const handleFontFileChange = async (e) => {
        ...
        try {
            ...
            // 后台上传到服务器（不阻塞 UI）
            fetch('/api/preferences/fonts', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ name: fontName, data: base64 }),
                credentials: 'include'
    ...
    const deleteCustomFont = async (fontName) => {
        try {
            ...
            // 后台从服务器删除
            fetch(`/api/preferences/fonts/${encodeURIComponent(fontName)}`, {
                method: 'DELETE',
                credentials: 'include'
    ...
    const renameCustomFont = async (oldName, newName) => {
        ...
        try {
            ...
            // 后台从服务器删除
            fetch(`/api/preferences/fonts/${encodeURIComponent(oldName)}`, {
                method: 'DELETE',
                credentials: 'include'
            }).catch(console.error);
            ...
            fetch('/api/preferences/fonts', {
                ...
                body: JSON.stringify({ name: newName, data: fontData.data }),
                credentials: 'include'
            }).catch(console.error);
    ...
    return (
        ...
        >
                    ...
                    <>
                        {[0, 1, 2].map(slotIndex => {
                            ...
                            return (
                                ...
                                >
                                    {slotData && (
                                        <div className="chapter-article w-full max-w-[960px] mx-auto mb-16"
            ...
            {bookFormat !== 'pdf' && (
                <div
                    className={`w-full shrink-0 mt-auto ${themes[theme].bg} ${themes[theme].text} flex items-center justify-between ${isIOS ? 'px-8' : 'px-4'} z-30 text-[10px] select-none transition-colors duration-300 ${showControls ? 'invisible' : 'opacity-80'}`}
                    style={{ paddingBottom: 'env(safe-area-inset-bottom)', height: 'calc(1.5rem + env(safe-area-inset-bottom))' }}
                >
                    <span className="truncate max-w-[70%] opacity-75">
                        {chapters[currentChapterIndex]?.title || bookTitle}
                    </span>
                    <span className="font-mono opacity-75">
                        {readingProgress}%  <span className="mx-1 opacity-50">/</span> {currentTime}
                    </span>
                </div>
            )}
            ...
            <div className={`fixed top-0 left-0 right-0 h-14 ${currentUiStyle} border-b backdrop-blur-md shadow-sm z-50 flex items-center px-4 justify-between transition-transform duration-300 ${showControls ? 'translate-y-0' : '-translate-y-full'} ${(isIOS && isFullscreen && !showControls) ? 'pointer-events-none opacity-0' : ''}`}>
            ...
            <div className={`fixed bottom-24 left-0 right-0 px-6 z-40 flex justify-between items-end transition-all duration-300 ${showControls ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0 pointer-events-none'}`}>
                <button
                ...
                <div className="flex flex-col gap-4 items-end">
                    <button
                        ...
                        className="w-10 h-10 rounded-full bg-black/70 text-white backdrop-blur-md flex items-center justify-center shadow-lg active:scale-95 transition-transform hover:bg-black/80"
                    >
            ...
            {/* Bottom Bar */}
            <div className={`fixed bottom-0 left-0 right-0 h-20 ${currentUiStyle} border-t backdrop-blur-md shadow-sm z-50 grid grid-cols-3 px-6 pb-4 items-center transition-transform duration-300 ${showControls ? 'translate-y-0' : 'translate-y-full'} ${(isIOS && isFullscreen && !showControls) ? 'pointer-events-none opacity-0' : ''}`}>
            ...
            {showControls && showSearch && (
                <div className={`fixed top-16 right-4 left-4 sm:left-auto sm:w-96 ${currentUiStyle} backdrop-blur-xl rounded-[24px] p-4 z-50 shadow-2xl animate-in slide-in-from-top-5 border border-white/10`}>
