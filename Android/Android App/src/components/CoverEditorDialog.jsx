...
const CoverEditorDialog = ({ isOpen, onClose, book, onSuccess, isDark, coverColors, getCoverUrl }) => {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className={`${colors.bg} w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]`}>
                ...
                <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center justify-center gap-6">
                    ...
                    >
                        {previewUrl ? (
                            <img src={getCoverUrl ? getCoverUrl(previewUrl) : previewUrl} alt="Preview" className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-xs text-gray-500">
                            支持 JPG, PNG 格式，建议尺寸 2:3
                        </p>
                    </div>
                </div>

                {/* Footer */}
                <div className={`px-6 py-4 border-t ${colors.border} flex items-center justify-between bg-gray-50/50`}>
                    <button
                        onClick={handleReset}
                        className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 transition-colors ${isDark ? 'text-gray-400 hover:bg-white/5' : ''}`}
                        title="恢复默认封面"
                    >
                    ...
                    <button
                        onClick={handleSave}
                        disabled={loading}
                        className="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm shadow-blue-500/30"
                    >
                        {loading ? '保存中...' : '保存更改'}
                    </button>
                </div>
