...
const Repository = () => {
    ...
    useEffect(() => {
        ...
        setUser(userData);
        // Initial fetch is handled by the dependency effect below
    }, []);
    ...
    const fetchData = useCallback(async (isLoadMore = false) => {
        ...
        try {
            // 1. Fetch Bookshelf Folders (Always needed for "Add to Bookshelf")
            if (bookshelfFolders.length === 0) {
                 const bsFoldersRes = await fetch('/api/books/bookshelf/folders', { headers, credentials: 'include' });
                 if (bsFoldersRes.ok) {
                     setBookshelfFolders(await bsFoldersRes.json());
                 }
            }
            
            // 2. Fetch Personal Folders (Needed for Personal Tab)
            if (folders.length === 0) {
                 const foldersRes = await fetch('/api/books/folders', { headers, credentials: 'include' });
                 if (foldersRes.ok) {
            ...
            const currentOffset = isLoadMore ? books.length : 0;
            let url = '';
            ...
            if (activeTab === 'public') {
                if (!currentFolderParam) {
                    // 1. 公共书库根目录：只获取书库列表
                    isPublicRoot = true;
                    const libsRes = await fetch('/api/books/public/libraries', { headers, credentials: 'include' });
                    if (libsRes.ok) {
                    ...
                    // 2. 进入公共书库子目录：请求书籍列表，此时拼接搜索参数
                    // 手动拆分 library 和 path (例如把 "默认书库/小说" 拆分开)
                    const parts = currentFolderParam.split('/');
                    ...
                    // 赋值给统一的 url 变量，交给下方的共有逻辑去请求
                    url = `/api/books/public?${params.toString()}`;
                }
            } else {
                // Personal Tab
                if (searchQuery) {
                     url = `/api/books/personal?search=${encodeURIComponent(searchQuery.trim())}&limit=${PAGE_SIZE}&offset=${currentOffset}`;
                } else {
                     const folderId = currentFolderParam; 
                     if (folderId) {
                         url = `/api/books/personal?folder_id=${folderId}&limit=${PAGE_SIZE}&offset=${currentOffset}`;
                     } else {
                         url = `/api/books/personal?root=1&limit=${PAGE_SIZE}&offset=${currentOffset}`;
                     }
                }
            }
            ...
            if (url && !isPublicRoot) {
                 ...
                 if (res.ok) {
                     ...
                     // 公共书库在请求书籍时，如果后端一并返回了当前目录的子文件夹，也更新一下
                     if (activeTab === 'public' && !isLoadMore && data.folders) {
                         setPublicFolders(data.folders);
                     }
                     ...
                     if (isLoadMore) {
                         setBooks(prev => {
                             ...
                             sessionStorage.setItem('repo_active_books_cache', JSON.stringify(merged));
                             return merged;
                         });
                     } else {
                         setBooks(newBooks);
                         sessionStorage.setItem('repo_active_books_cache', JSON.stringify(newBooks));
                     }
                     
                     setTotalBooks(data.total || 0);
                     // 判断是否还有更多：之前已有的数量 + 新拉取的数量 < 总数量
                     setHasMore(isLoadMore ? (books.length + newBooks.length < data.total) : (newBooks.length < data.total));
                 }
             }
    ...
    }, [activeTab, currentFolderParam, searchQuery, books.length, bookshelfFolders.length, folders.length]);

    // Effect to trigger fetch on nav change
    useEffect(() => {
        setHasMore(true);
        fetchData(false);
    ...
    const performBatchDelete = async () => {
        ...
        try {
            ...
            await Promise.all(Array.from(selectedBooks).map(id =>
                fetch(`/api/books/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` }, credentials: 'include' })
            ));
            // Delete Folders (Only in Personal Tab)
            if (activeTab === 'personal') {
                await Promise.all(Array.from(selectedFolders).map(id =>
                    fetch(`/api/books/folders/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` }, credentials: 'include' })
                ));
            }
    ...
    const deleteBook = async (id) => {
        const token = localStorage.getItem('token');
        try {
            const res = await fetch(`/api/books/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` }, credentials: 'include' });
            if (res.ok) {
                fetchData();
    ...
    const performRemoveFromBookshelf = async (bookId) => {
        ...
        try {
            const res = await fetch(`/api/books/bookshelf/${bookId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` }, credentials: 'include' });
    ...
    }, [activeTab, currentFolder, publicFolders, folders, searchQuery]);
    ...
    const renderHeader = () => (
        <div className={`px-4 py-3 sticky top-0 z-20 space-y-3 ${colors.headerBg}`}>
            ...
            {/* 修改判断条件：个人书库，或者进入了具体的公共书库子目录时，才显示搜索框 */}
    ...
    return (
        <div className={`min-h-full flex flex-col ${colors.bg} transition-colors duration-200 ${isEInk ? 'reader-theme-e-ink' : ''}`} style={{ paddingBottom: 'calc(6rem + env(safe-area-inset-bottom))' }}>
            {renderHeader()}
            ...
            {loading ? (
                ...
                <div className="px-4 space-y-6">
                    ...
                    {/* Empty State */}
                    {displayFolders.length === 0 && displayBooks.length === 0 && !loading && (
                    ...
                    <div ref={loadMoreRef} className="h-10 flex items-center justify-center">
                        {loadingMore && <Loader2 className="w-6 h-6 animate-spin text-gray-300" />}
                    </div>
                </div>
            )}
            ...
            {activeTab === 'personal' && !isSelectionMode && (
                <div className="fixed right-6 flex flex-col gap-3 z-40" style={{ bottom: 'calc(6.5rem + env(safe-area-inset-bottom))' }}>
                    {!currentFolder && (
                        <button
            ...
            {actionMenuTarget && (
                <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center backdrop-blur-sm" onClick={() => setActionMenuTarget(null)}>
                    <div className={`${colors.cardBg} w-full sm:w-80 rounded-t-2xl sm:rounded-2xl overflow-hidden px-2 pt-2 space-y-1 animate-in slide-in-from-bottom-10`} style={{ paddingBottom: 'calc(0.5rem + env(safe-area-inset-bottom))' }} onClick={e => e.stopPropagation()}>
