...
const ImageReader = ({ bookId, bookFormat, initialTheme, bookTitle, onBack, token }) => {
    // --- State ---
    ...
    // 阅读时长统计
    useReadingTime(bookId, token, activeTheme);
    ...
    useEffect(() => {
        if (bookFormat === 'pdf') {
            // PDF: Don't need to fetch TOC, just allow Document to render
            setLoading(false);
        } else {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (data.toc && data.toc.length > 0) {
                            ...
                            if (data.in_bookshelf !== undefined) setInBookshelf(data.in_bookshelf);
                        }
                    }
                } catch (e) {
                    console.error('Comic TOC load failed', e);
                    showToast.error('资源加载失败');
                } finally {
                    setLoading(false);
                }
            };
    ...
    const saveProgress = useCallback((pageNum, force = false) => {
        if (!token || !pageNum || !numPagesRef.current) return;
    ...
    useEffect(() => {
        ...
        const preloadImage = (pageNum) => {
            ...
            const img = new Image();
            const rawUrl = `/api/books/${bookId}/image?path=${encodeURIComponent(page.path)}&token=${token}`;
            const imageUrl = getAbsoluteUrl(rawUrl);
            img.src = imageUrl;
    ...
    // PDF Source
    const pdfUrl = getAbsoluteUrl(`/api/books/${bookId}/pdf_stream`);
    const pdfFile = useMemo(() => ({ url: pdfUrl, httpHeaders: { Authorization: `Bearer ${token}` } }), [pdfUrl, token]);
    ...
    return (
        <div className={`relative w-full h-screen overflow-hidden flex flex-col ${themes[isDark ? 'dark' : 'light'].bg} select-none`}>
            ...
            {/* --- Top Navbar --- */}
            <div
                className={`fixed top-0 left-0 right-0 ${currentUiStyle} border-b border-gray-100/10 backdrop-blur-md shadow-sm z-50 flex items-center px-4 justify-between transition-transform duration-300 ${showControls ? 'translate-y-0' : '-translate-y-full'}`}
                style={{ paddingTop: 'env(safe-area-inset-top)', height: 'calc(3.5rem + env(safe-area-inset-top))' }}
            >
                <div className="flex items-center gap-2 min-w-0">
                    <button onClick={onBackHandler} className={`p-2 -ml-2 rounded-full hover:bg-black/5 ${currentTextStyle}`}>
            ...
            <div
                ...
                style={{
                    ...
                    cursor: fitMode === 'custom' ? 'grab' : 'default'
                }}
                onClick={(e) => {
                    // 如果有面板打开，关闭面板
                    if (activePanel) {
                ...
                {!loading && containerSize.width > 0 && (
                    bookFormat === 'pdf' ? (
                        <div
                            ...
                            style={{ minHeight: containerSize.height ? `${containerSize.height}px` : '100%' }}
                        >
                            {renderMode === 'native' ? (
                                <div className="flex flex-col items-center justify-center p-6 text-center">
                                    <p className={`mb-4 text-sm opacity-70 ${currentTextStyle}`}>
                                        使用系统外部应用打开 PDF 以修复乱码问题
                                    </p>
                                    <button
                                        onClick={() => window.open(pdfUrl, '_system')}
            ...
            <div className={`fixed left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full bg-black/60 text-white text-xs backdrop-blur-md z-40 pointer-events-none transition-opacity duration-300 ${showControls ? 'opacity-0' : 'opacity-100'}`}
            ...
            {/* --- Bottom Navbar --- */}
            <div className={`fixed bottom-0 left-0 right-0 ${currentUiStyle} border-t border-gray-100/10 backdrop-blur-md shadow-sm z-50 grid grid-cols-3 px-6 items-center transition-transform duration-300 ${showControls ? 'translate-y-0' : 'translate-y-full'}`}
