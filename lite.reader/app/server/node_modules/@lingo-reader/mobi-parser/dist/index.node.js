...
function saveResource(data, type, filename, imageSaveDir) {
  {
    ...
    const url = path.resolve(imageSaveDir, fileName);
    node_fs.writeFileSync(url, data);
    return url;
  }
}
const mobiEncoding = {
...
async function toArrayBuffer(file) {
  {
    return typeof file === "string" ? bufferToArrayBuffer(new Uint8Array(node_fs.readFileSync(file))) : bufferToArrayBuffer(file);
  }
}
...
function getFont(buf) {
  ...
  if (flags & 1) {
    try {
      return fflate.unzlibSync(array);
    } catch (e) {
      console.warn(e);
...
class Mobi {
  ...
  async innerInit() {
    ...
    if (tocChapterStr) {
      ...
      const tocAst = await shared.parsexml(wrappedChapterStr, {
...
exports.initKf8File = initKf8File;
exports.initMobiFile = initMobiFile;
